    ...
    with torch.no_grad():
        ...
        for song_id in range(TOTAL_SONGS):
            ...
            try:
                ...
                # ---- Rhythm condition (from MIDI beats) ----
                if "rhythm" in config["condition_type"]:
                    print("using rhythm condition")
                    beat_times_all, _ = calculate_beats_and_downbeats(midi_file)
                ...
                beat_times_gt_eval, _ = calculate_beats_and_downbeats(midi_file)
                ...
                print(f"  ERROR lyrics_{song_id}: {exc}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate backing tracks for the lyrics2melody dataset (200 songs)."
    )
    ...
    config["text_prompt"]     = ""   # unused – loaded per song from prompts.json
    main(config)
