...
import mido
def compute_melody_v2(stereo_audio: torch.Tensor, segment_starts=0) -> np.ndarray:
    """
    Args:
    ...
    if sr != 44100:
        audio = torchaudio.functional.resample(audio, orig_freq=sr, new_freq=44100)
        sr = 44100
    audio = audio[:, int(segment_starts*44100):int(2097152+segment_starts*44100)]
    if audio.shape[0] == 1:
...
def compute_melody(input_audio):
    ...
    wav = wav[:, 25*44100:2097152+25*44100]
    if sr != sample_rate:
...
def compute_dynamics(audio_file, hop_length=160, target_sample_rate=44100, cut=False, segment_starts=0):
    """
    ...
    waveform = waveform[:, int(segment_starts*44100):int(2097152+segment_starts*44100)]
    if cut:
...
def extract_melody_one_hot(audio_path,
    ...
    audio_mono = audio[:, :2097152].mean(dim=0)
...
def calculate_beats_and_downbeats(midi_file_path):
    """
    Calculate beat and downbeat times for a MIDI file in 4/4 time signature,
    assuming constant tempo, and return them in seconds.
    
    Parameters:
    midi_file_path (str): Path to the MIDI file.
    
    Returns:
    tuple: (beat_times, downbeat_times), where both are lists of floats representing times in seconds.
    """
    midi = mido.MidiFile(midi_file_path)
    
    # Find tempo events
    tempo_events = [msg for track in midi.tracks for msg in track if msg.type == 'set_tempo']
    
    if len(tempo_events) == 0:
        tempo = 500000  # default tempo: 120 BPM
    elif len(tempo_events) > 1:
        print("Warning: Multiple tempo changes detected. Using the first tempo for calculation.")
    ...
    # Generate downbeat times (every 4 beats for 4/4 time)
    downbeat_times = []
    t = 0
    while t <= total_time:
...
def create_activations_from_midi(midi_file_path, beat_times, downbeat_times):
    # Load the MIDI file
    midi = mido.MidiFile(midi_file_path)
    ...
    T = midi.length
    print("midi.length", T)
    # Compute frame rate and number of frames
    frame_rate = 2378 * 44100 / 2097152  # ≈ 50 frames per second
    n = int(round(T * frame_rate))       # Number of frames
    
    # Initialize activation arrays
    beat_activations = np.zeros((1, n))
    downbeat_activations = np.zeros((1, n))
    
    # Populate beat activations
    for t in beat_times:
        frame_index = int(round(t * frame_rate))
        if 0 <= frame_index < n:
            beat_activations[0, frame_index] = 1
    
    # Populate downbeat activations
    for t in downbeat_times:
        frame_index = int(round(t * frame_rate))
        if 0 <= frame_index < n:
            downbeat_activations[0, frame_index] = 1
    
    # Apply Gaussian smoothing without reducing peak values
    sigma = 5  # Controls the width of the Gaussian kernel (adjust as needed)
    smoothed_beat_acts = gaussian_filter1d(beat_activations[0], sigma=sigma)
    smoothed_downbeat_acts = gaussian_filter1d(downbeat_activations[0], sigma=sigma)
    
    # Ensure peak values remain 1 by normalizing only the spread (optional adjustment)
    # Find the original peak locations and enforce them to 1
    for t in beat_times:
        frame_index = int(round(t * frame_rate))
        if 0 <= frame_index < n:
            smoothed_beat_acts[frame_index] = 1
    for t in downbeat_times:
        frame_index = int(round(t * frame_rate))
        if 0 <= frame_index < n:
            smoothed_downbeat_acts[frame_index] = 1
    
    # Reshape back to (1, n) and concatenate
    smoothed_beat_acts = smoothed_beat_acts.reshape(1, -1)
    smoothed_downbeat_acts = smoothed_downbeat_acts.reshape(1, -1)
    activation = np.concatenate((smoothed_beat_acts, smoothed_downbeat_acts))[:, :2378]
    # print("Activation shape:", activation.shape)
    
    # Plot the smoothed activations in the same plot
    time = np.linspace(0, T, n)
    plt.figure(figsize=(12, 4))
    plt.plot(time[:2378], activation[0], color='blue', label='Beats')
    plt.plot(time[:2378], activation[1], color='red', label='Downbeats')
    plt.xlabel('Time (seconds)')
    plt.ylabel('Activation')
    plt.title('Smoothed Beat and Downbeat Activations')
    plt.legend()
    plt.ylim(-0.1, 1.1)
    plt.savefig('smoothed_activations.png')
    
    return activation
def create_activations_from_timestamps(beat_times, downbeat_times):
    # Load the MIDI file
    # midi = mido.MidiFile(midi_file_path)
