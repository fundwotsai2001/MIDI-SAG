...
def main(config):
    ...
    with torch.no_grad():
        ...
        score_chord = []
        score_rhythm = []
        id = "aizaixiatian_gt_beat_reverse"
        ...
        if os.path.exists(os.path.join(output_dir, f"mixed_{id}.wav")):
            ...
            # If the first structure is 'intro', generate the first non-intro
            # segment (verse) first so it can serve as the audio reference when
            # the intro is generated second.  Everything after segment 0 remains
            # in its original order.
            intro_first = (
                config['structure_tag'][0].lower() == 'intro'
                and len(config['structure_tag']) > 1
            )
            if intro_first:
                segments_list = [1, 0] + list(range(2, len(config['structure_tag'])))
                print("Intro-last mode: generating verse first, then intro, then rest in order.")
            else:
            ...
            for s, segments in enumerate(segments_list):
                ...
                if config["apadapter"]:
                    ...
                    _ref_end = None
                    # intro-last: when generating intro (segments=0) after verse
                    # (segments=1), use the beginning of the verse tensor as
                    # audio reference so the intro is stylistically consistent.
                    _is_intro_after_verse = (
                        intro_first
                        and segments == 0
                        and tensors['1'].numel() > 0
                    )
                    if "audio" in config["condition_type"] and (s != 0 or _is_intro_after_verse):
                        if _is_intro_after_verse:
                            # Place the verse at the BACK of the latent window so the model
                            # uses it as right-context when generating the intro at the FRONT.
                            # Window layout:
                            #   [  intro (generate)  |  verse (reference)  ]
                            #   0 ── intro_dur ─────────────────────────── 47.6s
                            _intro_dur_s = (config['structure_start_seconds'][1]
                                            - config['structure_start_seconds'][0])
                            _intro_latent_len = int(_intro_dur_s / (2097152 / 44100) * 1024)
                            # Use intro_dur worth of verse audio as the back reference
                            _verse_ref_samples = int(_intro_dur_s * 44100)
                            verse_ref_audio = tensors['1'][:, :_verse_ref_samples].unsqueeze(0).to(weight_dtype).cuda()
                            audio_condition = torch.zeros((1, 64, 1024), device="cuda")
                            audio_condition_ref = pipe.vae.encode(verse_ref_audio).latent_dist.sample()
                            # Insert at back: positions [intro_latent_len, intro_latent_len + ref_len]
                            _ref_end_latent = min(_intro_latent_len + audio_condition_ref.shape[2], 1024)
                            audio_condition[:, :, _intro_latent_len:_ref_end_latent] = audio_condition_ref[:, :, :_ref_end_latent - _intro_latent_len]
                            extracted_audio_condition = audio_condition
                            masked_extracted_audio_condition = torch.zeros_like(extracted_audio_condition)
                            _window_start_s = 0.0
                            _ref_end = _verse_ref_samples   # used only for audio_mid_s below
                            _ref_start = 0
                            print(f"[intro-last] verse reference placed at back of window (latent [{_intro_latent_len}:{_ref_end_latent}])")
                        else:
                    ...
                    if _is_intro_after_verse:
                        # Verse is at the BACK; model generates intro at [0, intro_dur].
                        # audio_mid_s marks the intro/verse boundary in the window.
                        audio_mid_s = _intro_dur_s
                    elif _ref_end is not None:
                        audio_mid_s = (_ref_end - _ref_start) / 44100
                    else:
                        audio_mid_s = (config['structure_start_seconds'][segments + 1]
                                       - config['structure_start_seconds'][segments])
                    ...
                    _seg_dur_samples = int(44100 * (config['structure_start_seconds'][segments + 1]
                                                     - config['structure_start_seconds'][segments]))
                    if _is_intro_after_verse:
                        # Verse reference is at [intro_dur, ...] in the latent window.
                        # The model generates the intro at [0, intro_dur] → take from start.
                        intro_slice = tensors[str(segments)][:, :_seg_dur_samples].cpu()
                        backing_audio = torch.cat((intro_slice, backing_audio), dim=1)
                        print(f"[intro-last] prepended intro [{config['structure_start_seconds'][segments]:.2f}s ~ {config['structure_start_seconds'][segments+1]:.2f}s]")
                    elif s == 0:
                        backing_audio = torch.cat((backing_audio, tensors[str(segments)][:, :_seg_dur_samples].cpu()), dim=1)
                        print(f"generate {config['structure_start_seconds'][segments]} ~ {config['structure_start_seconds'][segments + 1]} seconds")
                    else:
                        backing_audio = torch.cat((backing_audio, tensors[str(segments)][:, int(44100*audio_mid_s):int(44100*audio_mid_s) + _seg_dur_samples].cpu()), dim=1)
                        print(f"generate {config['structure_start_seconds'][segments]} ~ {config['structure_start_seconds'][segments + 1]} seconds")
                    # else:
